#+title:      Agent Protocol Standards: ACP, MCP, A2A 통합 분석
#+date:       [2025-10-14 Mon 14:00]
#+filetags:   :meta:protocol:standards:acp:mcp:a2a:
#+identifier: 20251014T140000
#+export_file_name: 20251014T140000-protocol-standards.md

* Agent Protocol Standards: ACP, MCP, A2A 통합 분석

**버전**: 1.0.0  
**상태**: 🔬 RESEARCH  
**업데이트**: 2025-10-14

#+begin_quote
"Standards enable interoperability, innovation, and sustainable ecosystems"
"표준은 상호운용성, 혁신, 지속 가능한 생태계를 가능하게 한다"
#+end_quote

** 🌐 에이전트 프로토콜 생태계

*** 개요

2024-2025년 사이에 AI 에이전트 통신을 위한 세 가지 핵심 프로토콜이 등장:

1. **MCP** (Model Context Protocol) - Anthropic, 2024
2. **ACP** (Agent Client Protocol) - Zed + Google, 2025-08
3. **A2A** (Agent2Agent Protocol) - Google + Linux Foundation, 2025-04

각 프로토콜은 서로 다른 문제를 해결하며 **상호 보완적**입니다.

*** 전체 그림

#+begin_src
┌─────────────────────────────────────────────────┐
│  User / Editor                                  │
│  (Emacs, VS Code, JetBrains, Zed...)          │
└─────────────────────────────────────────────────┘
                     ↕ ACP
┌─────────────────────────────────────────────────┐
│  AI Coding Agent                                │
│  (Claude Code, Cursor, Aider...)               │
└─────────────────────────────────────────────────┘
                     ↕ A2A
┌─────────────────────────────────────────────────┐
│  Domain Agents                                  │
│  (Family-Config, Work-Config, Health...)       │
└─────────────────────────────────────────────────┘
                     ↕ MCP
┌─────────────────────────────────────────────────┐
│  Tools & Resources                              │
│  (File system, APIs, Databases...)             │
└─────────────────────────────────────────────────┘
#+end_src

** 📋 프로토콜별 상세 분석

*** 1. ACP (Agent Client Protocol)

**** 개요

- **발표**: 2025년 8월, Zed Industries + Google Gemini CLI
- **목적**: 코드 에디터 ↔ AI 코딩 에이전트 통신 표준화
- **별칭**: "The LSP for AI Coding Agents"
- **최근**: JetBrains 2025년 10월 채택 발표

**** 기술 스택

#+begin_src yaml
기술 기반: JSON-RPC 2.0
통신 방식: Bidirectional streaming
전송: stdio, TCP, WebSocket
라이선스: Apache 2.0
라이브러리: Rust, TypeScript
#+end_src

**** 핵심 기능

1. **Streaming Updates**: 실시간 코드 변경 스트리밍
2. **Permissioned Tool Calls**: 권한 기반 도구 호출
3. **File Access**: 파일 시스템 접근 표준화
4. **Context Management**: 대화 컨텍스트 관리
5. **Multi-modal**: 텍스트, 코드, 이미지 지원

**** 메시지 예시

#+begin_src json
// Editor → Agent
{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "agent/execute",
  "params": {
    "task": "Refactor this function to use async/await",
    "context": {
      "file": "src/main.rs",
      "selection": {
        "start": {"line": 10, "column": 0},
        "end": {"line": 25, "column": 5}
      }
    }
  }
}

// Agent → Editor (Streaming)
{
  "jsonrpc": "2.0",
  "method": "editor/applyEdit",
  "params": {
    "edits": [{
      "file": "src/main.rs",
      "range": {...},
      "newText": "async fn fetch_data() -> Result<Data, Error> {\n..."
    }]
  }
}
#+end_src

**** 사용 사례

- Emacs ↔ Claude Code
- VS Code ↔ Cursor AI
- JetBrains IDEs ↔ AI Coding Agents
- Zed ↔ Gemini CLI

*** 2. MCP (Model Context Protocol)

**** 개요

- **발표**: 2024년, Anthropic
- **목적**: AI 모델 ↔ 도구/리소스 연결 표준화
- **철학**: "One model, many tools"
- **목표**: 모델에게 풍부한 컨텍스트 제공

**** 기술 스택

#+begin_src yaml
기술 기반: JSON-RPC 2.0
통신 방식: Bidirectional
전송: stdio, HTTP/SSE
프로토콜: 클라이언트-서버 아키텍처
#+end_src

**** 핵심 개념

1. **Resources**: 읽기 가능한 데이터 소스
   - 파일 시스템
   - 데이터베이스
   - API 응답
   - 문서

2. **Tools**: 실행 가능한 기능
   - 파일 작성
   - API 호출
   - 명령 실행
   - 계산

3. **Prompts**: 재사용 가능한 프롬프트 템플릿

4. **Sampling**: 모델이 LLM 호출 요청

**** MCP Server 예시

#+begin_src typescript
// MCP Server: File System
import { MCPServer } from '@modelcontextprotocol/sdk';

const server = new MCPServer({
  name: 'filesystem-server',
  version: '1.0.0'
});

// Resource 제공
server.resource({
  uri: 'file:///home/user/data.txt',
  name: 'User Data',
  mimeType: 'text/plain'
}, async () => {
  return {
    contents: [{
      uri: 'file:///home/user/data.txt',
      mimeType: 'text/plain',
      text: await readFile('/home/user/data.txt')
    }]
  };
});

// Tool 제공
server.tool({
  name: 'write_file',
  description: 'Write content to a file',
  inputSchema: {
    type: 'object',
    properties: {
      path: { type: 'string' },
      content: { type: 'string' }
    }
  }
}, async (args) => {
  await writeFile(args.path, args.content);
  return { success: true };
});

server.listen();
#+end_src

**** 사용 사례 (현재)

- Claude ↔ GitHub
- Claude ↔ Google Drive
- Claude ↔ Postgres
- Claude ↔ Slack
- Claude ↔ n8n (워크플로우)
- Claude ↔ Playwright (브라우저 자동화)

*** 3. A2A (Agent2Agent Protocol)

**** 개요

- **발표**: 2025년 4월, Google
- **거버넌스**: 2025년 6월, Linux Foundation
- **목적**: AI 에이전트 간 통신 및 협업 표준화
- **파트너**: 50+ (Atlassian, Box, Cohere, Intuit, MongoDB, PayPal, Salesforce, SAP, ServiceNow...)

**** 기술 스택

#+begin_src yaml
기술 기반: JSON-RPC 2.0 + HTTP(S)
통신 방식: Request-Response, Streaming, Push notifications
보안: Enterprise-grade auth (OpenAPI parity)
확장성: Long-running tasks (hours/days)
#+end_src

**** 핵심 개념

1. **Agent Card**: 에이전트 능력 광고 (JSON)
2. **Task ID**: 작업 추적을 위한 고유 식별자
3. **Multi-modal**: 텍스트, 오디오, 비디오
4. **Human-in-the-loop**: 장기 실행 작업에서 사람 개입

**** Agent Card 형식

#+begin_src json
{
  "agentId": "family-config-agent",
  "version": "1.0.0",
  "name": "Family Life Management Agent",
  "description": "Manages family travel, finance, education, events",
  
  "capabilities": [
    {
      "id": "family.travel.plan",
      "name": "Travel Planning",
      "description": "Plan family trips with budget optimization",
      "inputSchema": {
        "type": "object",
        "properties": {
          "destination": { "type": "string" },
          "dates": {
            "type": "object",
            "properties": {
              "start": { "type": "string", "format": "date" },
              "end": { "type": "string", "format": "date" }
            }
          },
          "budget": { "type": "number" },
          "participants": {
            "type": "array",
            "items": {
              "type": "object",
              "properties": {
                "role": { "type": "string" },
                "age": { "type": "number" }
              }
            }
          }
        },
        "required": ["destination", "dates", "budget"]
      },
      "outputSchema": {
        "type": "object",
        "properties": {
          "itinerary": { "type": "object" },
          "costs": { "type": "object" },
          "recommendations": { "type": "array" }
        }
      }
    },
    {
      "id": "family.events.remind",
      "name": "Event Reminders",
      "description": "Track and remind family events (ceremonies, birthdays)"
    },
    {
      "id": "family.finance.track",
      "name": "Finance Tracking",
      "description": "Track family expenses and budgets"
    }
  ],
  
  "supportedModalities": ["text", "markdown", "json"],
  "authentication": {
    "type": "bearer",
    "required": true
  },
  "rateLimit": {
    "requestsPerMinute": 10,
    "concurrentTasks": 3
  }
}
#+end_src

**** A2A 통신 흐름

#+begin_src
[Meta Agent]
   ↓ 1. Discover: GET /agent-card
   ← Agent Card (capabilities)
   
   ↓ 2. Initiate: POST /tasks
   ← Task ID + Acknowledgment
   
   ↓ 3. Monitor: GET /tasks/{id}
   ← Progress updates
   
   ↓ 4. Complete: GET /tasks/{id}
   ← Final result
#+end_src

**** 메시지 예시

#+begin_src json
// Meta Agent → Domain Agent: Task Initiation
POST /tasks HTTP/1.1
Content-Type: application/json
Authorization: Bearer <token>

{
  "taskId": "task_20251014_001",
  "capability": "family.travel.plan",
  "input": {
    "destination": "제주도",
    "dates": {
      "start": "2025-07-20",
      "end": "2025-07-27"
    },
    "budget": 3000000,
    "participants": [
      { "role": "parent", "age": 40 },
      { "role": "parent", "age": 38 },
      { "role": "child", "age": 8 },
      { "role": "child", "age": 5 }
    ]
  },
  "preferences": {
    "accommodationType": "family-friendly resort",
    "activities": ["nature", "beach", "culture"]
  }
}

// Domain Agent → Meta Agent: Acknowledgment
HTTP/1.1 202 Accepted
Content-Type: application/json

{
  "taskId": "task_20251014_001",
  "status": "accepted",
  "estimatedDuration": 45,
  "message": "Travel planning task accepted"
}

// Domain Agent → Meta Agent: Progress (streaming)
{
  "taskId": "task_20251014_001",
  "status": "in_progress",
  "progress": 0.6,
  "currentStep": "searching_accommodations",
  "message": "Found 12 accommodation options, filtering..."
}

// Domain Agent → Meta Agent: Completion
HTTP/1.1 200 OK
Content-Type: application/json

{
  "taskId": "task_20251014_001",
  "status": "completed",
  "result": {
    "itinerary": {
      "file": "~/org/family/travel/20250720-jeju.org"
    },
    "summary": {
      "totalCost": 2850000,
      "flightsFound": 5,
      "accommodationsFound": 12,
      "activitiesPlanned": 15
    },
    "recommendations": [
      "Book flights by 2025-04-30 to save 20%",
      "Book accommodation by 2025-05-15 (peak season)"
    ]
  }
}
#+end_src

**** 사용 사례

- Meta Agent ↔ Domain Agents (Family, Work, Health...)
- Research Agent ↔ Analysis Agent ↔ Report Agent
- Customer Service Agent ↔ Order Agent ↔ Shipping Agent
- Multi-agent 협업 시스템

** 🎯 Family-Config에 적용

*** 계층적 아키텍처

#+begin_src
┌─────────────────────────────────────────────────┐
│  Layer 5: User Interface                        │
│  - Emacs/Org-mode                               │
│  - Natural Language                             │
└─────────────────────────────────────────────────┘
                     ↕ ACP
┌─────────────────────────────────────────────────┐
│  Layer 4: Meta Agent (claude-config)            │
│  - Claude Code                                  │
│  - 전체 오케스트레이션                           │
│  - 도메인 라우팅                                 │
└─────────────────────────────────────────────────┘
                     ↕ A2A
┌─────────────────────────────────────────────────┐
│  Layer 3: Domain Agents                         │
│  ┌───────────────────────────────────────────┐  │
│  │ Family-Config Agent                       │  │
│  │ - Agent Card: family.* capabilities       │  │
│  │ - A2A Server                              │  │
│  └───────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────┐  │
│  │ Work-Config Agent (미래)                  │  │
│  │ - Agent Card: work.* capabilities         │  │
│  └───────────────────────────────────────────┘  │
└─────────────────────────────────────────────────┘
                     ↕ MCP
┌─────────────────────────────────────────────────┐
│  Layer 2: MCP Tools                             │
│  - web_search (정보 검색)                       │
│  - n8n (워크플로우)                             │
│  - playwright (브라우저)                        │
│  - filesystem (파일 시스템)                     │
│  - calendar (Google Calendar)                  │
│  - supabase (데이터베이스)                      │
└─────────────────────────────────────────────────┘
                     ↕ HTTP/APIs
┌─────────────────────────────────────────────────┐
│  Layer 1: External Services                     │
│  - Google (Flights, Calendar, Drive)           │
│  - Booking.com (숙박)                          │
│  - Airlines APIs                               │
│  - Banking APIs                                │
└─────────────────────────────────────────────────┘
#+end_src

*** 프로토콜 역할 분담

| Layer | 프로토콜 | 목적 | 예시 |
|-------|---------|------|------|
| 5 ↔ 4 | ACP | 사용자 ↔ Meta Agent | Emacs ↔ Claude Code |
| 4 ↔ 3 | A2A | Meta Agent ↔ Domain Agents | Claude Code ↔ Family-Config |
| 3 ↔ 2 | MCP | Domain Agent ↔ Tools | Family-Config ↔ web_search |
| 2 ↔ 1 | HTTP | Tools ↔ Services | web_search ↔ Google APIs |

*** 실제 통신 예시

**** 사용자 요청

#+begin_src
사용자 (Emacs): "여름에 제주도 가족 여행 계획 좀 도와줘"
#+end_src

**** 1. ACP: Emacs → Claude Code

#+begin_src json
// ACP Message
{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "agent/execute",
  "params": {
    "task": "여름에 제주도 가족 여행 계획 좀 도와줘",
    "context": {
      "workingDirectory": "/home/junghan/org/family",
      "openFiles": ["travel/plans.org"]
    }
  }
}
#+end_src

**** 2. Meta Agent 분석

#+begin_src python
# Meta Agent (Claude Code) 내부
intent = analyze("여름에 제주도 가족 여행 계획")
# → domain: "family", task_type: "travel.plan"

# A2A Agent Discovery
agent_card = fetch("http://localhost:8080/agent-card")
# → Family-Config Agent의 capabilities 확인
#+end_src

**** 3. A2A: Meta Agent → Family-Config Agent

#+begin_src json
// A2A Task Initiation
POST http://localhost:8080/tasks
Content-Type: application/json

{
  "taskId": "task_20251014_001",
  "capability": "family.travel.plan",
  "input": {
    "destination": "제주도",
    "dates": { "start": "2025-07-20", "end": "2025-07-27" },
    "budget": 3000000,
    "participants": [
      { "role": "dad" },
      { "role": "mom" },
      { "role": "child", "age": 8 },
      { "role": "child", "age": 5 }
    ]
  }
}

// Response: Accepted
{
  "taskId": "task_20251014_001",
  "status": "accepted",
  "estimatedDuration": 45
}
#+end_src

**** 4. MCP: Family-Config Agent → Tools

#+begin_src typescript
// Family-Config Agent 내부에서 MCP 호출

// 1. 항공권 검색
const mcpClient = new MCPClient();
const flights = await mcpClient.callTool({
  server: "web-search-mcp",
  tool: "search",
  arguments: {
    query: "서울-제주 항공권 2025-07-20 왕복 4명",
    limit: 10
  }
});

// 2. 숙박 검색
const accommodations = await mcpClient.callTool({
  server: "web-search-mcp",
  tool: "search",
  arguments: {
    query: "제주도 가족 친화적 리조트 2025-07-20 7박",
    limit: 15
  }
});

// 3. 과거 여행 데이터 조회
const pastTrips = await mcpClient.callTool({
  server: "filesystem-mcp",
  tool: "read_file",
  arguments: {
    path: "~/org/family/travel/history/"
  }
});

// 4. Calendar 동기화
await mcpClient.callTool({
  server: "google-calendar-mcp",
  tool: "create_event",
  arguments: {
    title: "제주도 가족 여행",
    start: "2025-07-20T09:00:00+09:00",
    end: "2025-07-27T18:00:00+09:00"
  }
});
#+end_src

**** 5. A2A: Progress Updates

#+begin_src json
// Family-Config Agent → Meta Agent (streaming)
{
  "taskId": "task_20251014_001",
  "status": "in_progress",
  "progress": 0.3,
  "message": "Found 5 flight options"
}

{
  "taskId": "task_20251014_001",
  "status": "in_progress",
  "progress": 0.6,
  "message": "Filtering 12 accommodations"
}
#+end_src

**** 6. A2A: Final Result

#+begin_src json
// Family-Config Agent → Meta Agent
{
  "taskId": "task_20251014_001",
  "status": "completed",
  "result": {
    "file": "~/org/family/travel/20250720-jeju.org",
    "summary": {
      "totalCost": 2850000,
      "withinBudget": true,
      "flightsFound": 5,
      "accommodationsFound": 12,
      "recommendedFlight": "진에어 09:30 출발",
      "recommendedAccommodation": "힐튼 제주 리조트"
    },
    "nextActions": [
      {
        "action": "book_flight",
        "deadline": "2025-04-30",
        "estimatedCost": 680000
      },
      {
        "action": "book_accommodation",
        "deadline": "2025-05-15",
        "estimatedCost": 1400000
      }
    ]
  }
}
#+end_src

**** 7. ACP: Claude Code → Emacs

#+begin_src json
// ACP Response
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": {
    "message": "제주도 여행 계획이 완성되었습니다!",
    "files": [
      {
        "path": "~/org/family/travel/20250720-jeju.org",
        "action": "created"
      }
    ],
    "summary": "총 예산 285만원 (목표 300만원 이내)\n항공권 5개 옵션, 숙박 12개 옵션 발견\n다음 단계: 항공권 예약 (마감: 4월 30일)"
  }
}
#+end_src

** 💡 핵심 인사이트

*** 1. 프로토콜 역할 분리

- **ACP**: Human ↔ Agent (코딩 작업 중심)
- **A2A**: Agent ↔ Agent (협업 및 위임)
- **MCP**: Agent ↔ Tools (실행 및 데이터 접근)

→ 명확한 책임 분리로 각각 독립적으로 발전 가능

*** 2. JSON-RPC 2.0 표준

세 프로토콜 모두 JSON-RPC 2.0 기반:
- 성숙한 표준
- 다양한 언어 지원
- 툴링 풍부
- 학습 곡선 낮음

*** 3. 상호 보완성

#+begin_src
MCP (2024) → A2A (2025-04) → ACP (2025-08)

시간순으로 등장했지만, 각각 다른 레이어 담당
→ 계층적으로 쌓아올릴 수 있음
#+end_src

*** 4. 오픈 스탠다드

모두 오픈소스 & 오픈 스탠다드:
- MCP: Anthropic (MIT License)
- A2A: Linux Foundation
- ACP: Zed + Google (Apache 2.0)

→ Vendor lock-in 없음, 커뮤니티 주도 발전

*** 5. Enterprise-Ready

- Authentication/Authorization
- Rate limiting
- Long-running tasks
- Error handling
- Monitoring/Observability

→ 프로토타입뿐만 아니라 프로덕션 사용 가능

** 🚀 구현 전략

*** Phase 1: MCP 통합 (현재)

Family-Config Agent가 MCP 클라이언트로:
- web_search MCP 서버 연결
- filesystem MCP 서버 연결
- calendar MCP 서버 연결

#+begin_src typescript
// family-config/src/mcp-client.ts
import { Client } from '@modelcontextprotocol/sdk/client/index.js';

class FamilyConfigMCPClient {
  async searchFlights(query: string) {
    return await this.client.callTool({
      name: 'web_search',
      arguments: { query, limit: 10 }
    });
  }
  
  async readPastTrips() {
    return await this.client.request({
      method: 'resources/read',
      params: { uri: 'file://~/org/family/travel/history/' }
    });
  }
}
#+end_src

*** Phase 2: A2A 서버 구현

Family-Config Agent를 A2A 서버로 노출:

#+begin_src typescript
// family-config/src/a2a-server.ts
import express from 'express';

const app = express();

// Agent Card 제공
app.get('/agent-card', (req, res) => {
  res.json({
    agentId: 'family-config-agent',
    capabilities: [
      { id: 'family.travel.plan', ... },
      { id: 'family.finance.track', ... },
      { id: 'family.events.remind', ... }
    ]
  });
});

// Task 수신
app.post('/tasks', async (req, res) => {
  const { taskId, capability, input } = req.body;
  
  // 즉시 수락
  res.status(202).json({
    taskId,
    status: 'accepted',
    estimatedDuration: 45
  });
  
  // 백그라운드 실행
  processTask(taskId, capability, input);
});

// Task 상태 조회
app.get('/tasks/:id', (req, res) => {
  const status = getTaskStatus(req.params.id);
  res.json(status);
});

app.listen(8080);
#+end_src

*** Phase 3: ACP 클라이언트 (미래)

Emacs에서 ACP 클라이언트 구현:

#+begin_src elisp
;; emacs/acp-client.el
(defun acp-call-agent (task)
  "Call AI agent via ACP protocol"
  (let* ((request (json-encode
                   `((jsonrpc . "2.0")
                     (id . 1)
                     (method . "agent/execute")
                     (params . ((task . ,task)
                                (context . ((workingDirectory . ,default-directory))))))))
         (process (make-network-process
                   :name "acp-client"
                   :host "localhost"
                   :service 5000
                   :coding 'utf-8
                   :filter 'acp-response-handler)))
    (process-send-string process (concat request "\n"))))
#+end_src

** 📚 참고 자료

*** 공식 사이트

- ACP: https://agentclientprotocol.com
- MCP: https://modelcontextprotocol.io
- A2A: https://a2aprotocol.ai

*** GitHub 리포지토리

- ACP: https://github.com/zed-industries/agent-client-protocol
- MCP: https://github.com/modelcontextprotocol
- A2A: https://github.com/a2aproject/A2A

*** 논문

- "A Survey of Agent Interoperability Protocols" (arXiv:2505.02279v1)

** 🎓 결론

*** 기술적 완성도

세 프로토콜을 계층적으로 통합하면:
- **표준 기반**: 업계 표준 준수
- **상호운용성**: 다양한 에이전트/도구 연결
- **확장성**: 새로운 도메인/도구 쉽게 추가
- **유지보수성**: 명확한 책임 분리
- **미래 지향적**: 커뮤니티 주도 발전

*** Family-Config의 위치

"개인용이지만 기술적으로 완전한 에이전트 활용 사례":
- ✅ 표준 프로토콜 기반 (ACP, A2A, MCP)
- ✅ 계층적 아키텍처 (Meta ↔ Domain ↔ Tools)
- ✅ 분산 서비스 활용 (각 에이전트가 독립 서비스)
- ✅ 프로덕션 품질 (인증, 에러 처리, 모니터링)
- ✅ 오픈소스 & 재사용 가능 (템플릿으로 배포)

이는 **개인 프로젝트를 넘어선 기술적 참고 구현**이 될 수 있습니다.

---

**상태**: 🔬 RESEARCH → 🛠️ IMPLEMENTATION  
**다음 단계**: 실제 A2A 서버 구현 (family-config)

#+end_src
